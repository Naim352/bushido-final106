...
// envoie via une API HTTPS classique, qui elle n'est jamais bloquée.
// ══════════════════════════════════════════════
const RESEND_API_KEY = process.env.RESEND_API_KEY;
// Tant que ton propre domaine n'est pas vérifié sur resend.com,
// on doit envoyer depuis cette adresse fournie par Resend.
const RESEND_FROM = process.env.RESEND_FROM || 'Bushido Store <onboarding@resend.dev>';

async function sendMail(to, subject, html) {
  if (!RESEND_API_KEY) {
    console.warn('⚠️  Email non envoyé (RESEND_API_KEY manquant) :', subject);
    return;
  }
  try {
    ...
    if (!r.ok) {
      ...
      console.error('Erreur envoi email (Resend):', r.status, errText);
    }
  } catch (err) {
...
async function sendMailVerif(to, subject, html) {
  ...
  try {
    ...
    if (!r.ok) {
      const errText = await r.text();
      console.error('Erreur envoi email vérification (Resend):', r.status, errText);
      return false;
    }
    return true;
  } catch (err) {
...
// ⚠️ /api/webhook est exclu ici : Stripe a besoin du corps BRUT (non parsé)
// pour vérifier la signature. Le parsing raw est fait directement sur cette
// route (voir plus bas), donc on ne doit pas la faire passer par express.json().
...
app.post('/api/auth/inscription', async (req, res) => {
  ...
  try {
    ...
    if (!emailEnvoye) {
      return res.status(500).json({ error: 'Impossible d\'envoyer l\'email de vérification. Vérifie que RESEND_API_KEY est configuré sur Railway.' });
    }
...
app.post('/api/create-payment-intent', async (req, res) => {
  const { amount, currency = 'eur', email, name, articles, phone, address, zip, city } = req.body;
  ...
  if (event.type === 'payment_intent.succeeded') {
    ...
    if (email) {
      try {
        ...
        const result = await db.query('SELECT id, prenom, nom, telephone, adresse, code_postal, ville FROM clients WHERE email=$1', [email.toLowerCase()]);
        if (result.rows.length > 0) {
          const clientId = result.rows[0].id;
          const [prenom, ...rest] = name.split(' ');

          // Enregistrer la commande
          const articles = pi.metadata?.articles || 'Détail non disponible';
          await db.query(
            `INSERT INTO commandes (client_id, stripe_id, montant, articles, statut)
             VALUES ($1, $2, $3, $4, 'En traitement')`,
            [clientId, pi.id, amount, articles]
          );
          ...
          if (process.env.ADMIN_EMAIL) {
            ...
            const { telephone, adresse, code_postal, ville } = result.rows[0];
            // Priorité aux infos saisies lors du paiement, sinon celles du profil
            const telFinal = pi.metadata?.phone || telephone || 'Non renseigné';
            const adrFinal = pi.metadata?.address || adresse;
            const zipFinal = pi.metadata?.zip || code_postal;
            const villeFinal = pi.metadata?.city || ville;
            const adresseComplete = adrFinal
              ? `${adrFinal}${zipFinal ? ', ' + zipFinal : ''}${villeFinal ? ' ' + villeFinal : ''}`
              : '⚠️ Non renseignée (à demander au client)';
            sendMail(
              process.env.ADMIN_EMAIL,
              `💰 Paiement reçu : ${amount.toFixed(2)} € — ${nomClient}`,
              `<h2>Nouveau paiement confirmé</h2>
               <p><b>Client :</b> ${nomClient} (${email})</p>
               <p><b>Téléphone :</b> ${telFinal}</p>
               <p><b>Adresse de livraison :</b> ${adresseComplete}</p>
               <p><b>Articles commandés :</b> ${articles}</p>
...
app.post('/api/auth/mot-de-passe-oublie', async (req, res) => {
  ...
  try {
    ...
    if (!envoye) return res.status(500).json({ error: 'Impossible d\'envoyer l\'email. Vérifie RESEND_API_KEY sur Railway.' });
...
app.post('/api/admin/invitation', async (req, res) => {
  ...
  if (!envoye) return res.status(500).json({ error: 'Impossible d\'envoyer l\'invitation. Vérifie RESEND_API_KEY sur Railway.' });
